...
namespace uscp::wcrossover
{
	struct min final
	{
		explicit min(const uscp::problem::instance& problem_): problem(problem_)
		{
		}
		min(const min&) = default;
		min(min&&) noexcept = default;
		min& operator=(const min& other) = delete;
		min& operator=(min&& other) noexcept = delete;
